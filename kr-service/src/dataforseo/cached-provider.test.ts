...
test("cache: la segunda corrida NO vuelve a pedir NADA (costo cero)", async () => {
  ...
  await new CachedProvider(spy, cache).searchVolume(kws, M);
  ...
  const res = await new CachedProvider(spy2, cache).searchVolume(kws, M);
...
test("cache: con una keyword nueva se pide SOLO esa, no el lote entero", async () => {
  ...
  await new CachedProvider(new SpyProvider(conDatos), cache).searchVolume(["a", "b", "c"], M);

  const spy = new SpyProvider(conDatos);
  const res = await new CachedProvider(spy, cache).searchVolume(["a", "b", "c", "nueva"], M);
...
test("cache: se cachea el 'no tengo dato' (si no, se paga por la nada una y otra vez)", async () => {
  ...
  await new CachedProvider(new SpyProvider(), cache).bulkKeywordDifficulty(sinDatos, M);

  const spy = new SpyProvider();
  const kd = await new CachedProvider(spy, cache).bulkKeywordDifficulty(sinDatos, M);
...
test("cache: la ausencia caduca ANTES que la presencia (puede aparecer volumen después)", async () => {
  const cache = new MemCache();
  await new CachedProvider(new SpyProvider(new Set(["con datos"])), cache).bulkKeywordDifficulty(
    ["con datos", "sin datos"],
    M,
  );

  const ttlPresente = cache.ttls.get(cacheKeys.keywordDifficulty("con datos", M));
  const ttlAusente = cache.ttls.get(cacheKeys.keywordDifficulty("sin datos", M));
...
test("cache: el mismo keyword en OTRO mercado NO es un hit (clave completa, ADR-10)", async () => {
  ...
  await new CachedProvider(new SpyProvider(new Set(["pizza"])), cache).searchVolume(["pizza"], M);

  const spy = new SpyProvider(new Set(["pizza"]));
  await new CachedProvider(spy, cache).searchVolume(["pizza"], otroMercado);
...
test("cache: un SERP con otra profundidad NO es un hit", async () => {
  const cache = new MemCache();
  await new CachedProvider(new SpyProvider(), cache).serp("pizza", M, 10);

  const spy = new SpyProvider();
  await new CachedProvider(spy, cache).serp("pizza", M, 20);
...
test("cache: variantes de casing son el MISMO dato (clave canónica)", async () => {
  const cache = new MemCache();
  await new CachedProvider(new SpyProvider(new Set(["Pizza Napolitana Madrid"])), cache).searchVolume(
    ["Pizza Napolitana Madrid"],
    M,
  );

  const spy = new SpyProvider();
  await new CachedProvider(spy, cache).searchVolume(["pizza napolitana madrid"], M);
...
test("cache: las stats reportan cuántas consultas se ahorraron", async () => {
  ...
  await new CachedProvider(new SpyProvider(conDatos), cache).searchVolume(["a", "b"], M);

  const p = new CachedProvider(new SpyProvider(conDatos), cache);
