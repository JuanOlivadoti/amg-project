...
| Qué | Dónde | Nota |
|---|---|---|
| **El portal de la agencia** — 4 piezas desde `dashboard-project` | [**programa**](../superpowers/plans/2026-08-01-portal-agencia-programa.md) · piezas: [clientes](../superpowers/plans/2026-08-01-paginas-clientes-portal.md) · [usuarios](../superpowers/plans/2026-08-01-paginas-usuarios-portal.md) · [ideas](../superpowers/plans/2026-08-01-modulo-ideas-portal.md) · [dashboard](../superpowers/plans/2026-08-01-dashboard-home-portal.md) | 🟡 **En curso, dos de cuatro piezas con su `db`+`api` listos, ninguna mergeada a `main` todavía.** Se ejecuta **en otra máquina/sesión y una rama por pieza**, en paralelo a la demo: `main` sigue siendo del roadmap. Orden **clientes → usuarios → ideas → dashboard**. **Pieza 1 (clientes)**: `db`+`api`+portal completos en `feature/paginas-clientes` (ver el bloque "Nuevo (2026-08-02)" más arriba). **Pieza 2 (usuarios)**: Etapas 1 y 2 (`db`+`api` — lectura de miembros y cambio de rol) commiteadas en `feature/paginas-usuarios`; **falta el portal** (pantallas de usuarios, etapas siguientes). Migraciones reservadas `0011`/`0012`/`0013` para que las ramas no choquen — `0011` (clientes) y `0012` (usuarios) ya escritas, ninguna desplegada aún. Decisiones cerradas al planificar, ambas confirmadas al construir: **usuarios gestiona membresías, no altas** (la API no recibe ninguna credencial de Supabase, verificado en `scripts/env-sync.mts:26`), **los permisos son derivados del rol y read-only** (preserva ADR-15 — cambiar el rol de otro sigue siendo exclusivo de `maestro`, impuesto por RLS, no por un `if`). La allowlist del renderizador **no crece**: lo que traen estas piezas es interno (teléfonos, notas, contratos, transcripciones). |
