...
>
> Última actualización: **2026-08-06** · **1069 tests en verde** — **786** en el monorepo + **283** en
> el portal (205 `node:test` + 78 Karma). Subió de 766 al mergear la pieza 2 (usuarios); los dos
...
> últimos de la 13ª review externa (ver [§ tanda 19](08-testing-calidad.md)).
> Medido con `npm run verificar` más los dos suites del portal.
>
...
> del dataset ya es durable** (`datasets/`, con un test que se lo pregunta a `git check-ignore`).
>
> ✅ **Actualizado el 2026-08-04: el orden que produce el pipeline YA llega al portal.** Era lo que
> deshacía `db/src/store.ts` con su `order by opportunity_score`; ahora el orden es un dato
> (`kr_pages.orden_brief`, migración `0015`) y las dos lecturas ordenan por una única definición.
> `kr-service` y `orchestrator` no cambiaron: el array ya viajaba ordenado. Y **el portal no necesitó
> cambios** — lo que este documento decía de `portal/src/app/core/cartera.ts` era **falso**: ese `sort`
> es `topOportunidades`, el widget de "las N de mayor score", con otro propósito;
> `separarPorEvidencia` preserva el orden de entrada. Lo que sigue abierto de KR-3 es solo el **dato**
> (~$0.31, decide Juan) y la **calibración** de los dos parámetros nuevos.
>
...
> [09-estado-y-roadmap.md](09-estado-y-roadmap.md). **1069 tests** (786 monorepo + 283
...
| **KR-2 — el informe legible del research, en el portal** | [spec](../superpowers/specs/2026-08-04-informe-kr-portal-design.md) · [plan KR-2a](../superpowers/plans/2026-08-05-kr2a-paquete-contrato.md) | ✅ **Completo: KR-2a el 2026-08-05, KR-2b el 2026-08-06** ([plan KR-2b](../superpowers/plans/2026-08-05-kr2b-informe-en-el-portal.md)). Era la pieza que faltaba para la demo del módulo de Keyword Research: `renderReport` producía el mejor entregable del módulo y **solo existía como archivo local** tras correr el CLI. **KR-2a** extrajo el contrato del brief al 7º workspace `contrato/` (tipos + los **dos** validadores Zod + `renderReport`), cerró la deuda del Zod duplicado y arregló tres cosas del informe: `n/d` en vez de `NaN`, la tabla de desglose que no se pinta sin datos, y el **escapado de delimitadores** de Markdown (bug que ya existía en el CLI). 11 commits, 9 tareas con review cada una, **734 tests**. **KR-2b** cerró lo demás en **17 commits** y 7 tareas con review cada una: la migración `0016` (`kr_informes`, política `app.es_staff()` **y sus grants**), `guardarInforme`/`getInforme`, el step del orquestador **antes de `cerrar-run`** (que hace enunciable *un run en `pending_approval` siempre tiene informe*), los dos endpoints con el `filename` por allowlist, el seed de la demo con sus tres huecos en `n/d`, y la pantalla con parser propio **sin `innerHTML`**. Decisiones: el informe es **interno** de la agencia (lleva el coste, que ante Frank es argumento de venta), **pantalla + descarga `.md`**, el `.md` **guardado** ya renderizado, y el **PDF trasladado** al entregable del restaurante (nota fechada en ADR-07). |
