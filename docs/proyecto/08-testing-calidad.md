...
## Cobertura actual: 1368 tests (monorepo) + 344 (portal)

> Las cifras de esta tabla se miden con `npm run verificar`, que las cuenta de la salida de
> `node:test`. Si no coinciden, la que está mal es la tabla. Última medición: **2026-08-10**.
>
> ⚠️ La del 2026-08-10 es **suma por paquete**, no una corrida de `verificar` en verde: `db` y `api`
> medidos entonces, el resto de la última corrida limpia, porque había trabajo sin commitear de otra
> sesión en `web-builder/`. Se re-mide entero cuando aterrice.
...
**Cuatro revisiones, cuatro veredictos de CAMBIOS_PEDIDOS.** Ninguna aprobó a la primera, y eso es la
