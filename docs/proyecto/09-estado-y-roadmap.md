...
| **Tests** | **1712** — 1368 en el monorepo + 344 en el portal (237 `node:test` + 107 Karma). Los de seguridad, contra Postgres real. Cubre los bloques A (**A4 incluido**), B y **C entero**, el **bloque E completo** (entregas 1, 2 y 3, con el gate de paridad re-capturado al cerrar) y las **etapas 1-4 del bloque J** (Ideas: la `0013`, `PgIdeas`, los tres endpoints y el seed de ejemplo) (venía de 1199 → … → 1612 → 1659 → 1693 → 1712). El portal, con `-- --con-portal` y Karma aparte. ⚠️ **La cifra del 2026-08-10 es una suma por paquete, no una corrida de `npm run verificar` en verde**: `db` 313 y `api` 184 medidos en esta corrida, el resto tomado de la última corrida limpia, porque había trabajo sin commitear de otra sesión en `web-builder/` que dejaba el paso 5 en ✖. Se re-mide entero cuando ese trabajo aterrice. |
| **Migraciones** | **19 en `main`, 18 aplicadas en producción** (`0001`..`0012` + `0014`..`0019`), la `0018` y la `0019` el **2026-08-08**, la **`0014` el 2026-08-08 por la noche**. **Pendiente de aplicar: la `0013` (Ideas)**, escrita el 2026-08-09 — el número estaba reservado para la pieza 3 del portal y dejó de estarlo. Aplica **al final**, después de la `0019`, y que eso no cambie el esquema resultante lo prueba un test propio (no depende de ninguna hermana: solo consume piezas ≤ `0012`). La próxima libre es la **`0020`**. **La `0014` aplicó en la posición que el test de orden predecía** —entre la `0012` y la `0015`, no al final— y **el `grant` sobrevivió al `drop column`**, verificado donde importa: `amg-renderer-production.up.railway.app` sirve «La Birra Bar» con las dos direcciones del footer, y ese dato solo puede venir de `business_profile_publico`. **El primer intento de la `0018` falló** (`must be able to SET ROLE "app_barrido"`) y se revirtió sola: en PGlite el rol que migra es superusuario y en Supabase alojado no, así que `alter … owner to` exigía dos permisos que nadie había concedido. Arreglado y con un test que aplica esa migración **como rol no-superusuario** — sin él, la clase entera volvería a escaparse |
...
| ✅ | **1349 tests en verde** (+344 en el portal: 237 `node:test` + 107 Karma) + typecheck limpio en los 7 paquetes. Los de seguridad, contra Postgres real. |
