...
| **Tests** | **1541** — 1197 en el monorepo + 344 en el portal (237 `node:test` + 107 Karma). Los de seguridad, contra Postgres real. Medido el **2026-08-08** con `npm run verificar` tras la 15ª review, los bloques A (**A4 incluido**), B y **C entero**, y del bloque E las **entregas 1 y 2** más las **mitades A y C de la 3** (venía de 1199 → … → 1502 → 1521 → 1541). El portal, con `-- --con-portal` y Karma aparte. |
...
| ✅ | **1197 tests en verde** (+344 en el portal: 237 `node:test` + 107 Karma) + typecheck limpio en los 7 paquetes. Los de seguridad, contra Postgres real. |
