...
> antes no lo hacía.
>
> ⚠️ **Verificación en navegador (C6 de `CHECKPOINTS.md`) queda pendiente** — bloqueada por el entorno
> de esta sesión: `chrome-devtools-mcp` reportó el perfil de Chrome ya en uso por otro proceso, sin
> forma segura de liberar el lock sin arriesgar cerrar ventanas del usuario. Se generó el HTML real vía
> `renderMenu()` con un perfil de prueba (video, alérgenos, etiquetas, nutrición, comensales) para
> inspección estática, pero falta confirmar en un navegador real: los controles nativos del `<video>` a
> 160×120px (subido desde 72×72 porque el tamaño de una miniatura de foto no alcanza para que se vean
> usables), los badges de alérgenos/etiquetas en claro y oscuro, el `<details>` de nutrición por
> teclado, y que `comensales` no desborde a 390px. `web-builder`: 417/417. `db`: 358/358 (incluye el
> ajuste de independencia de orden de `fotos-publicas.test.ts`, que ahora generaliza a una lista las
> migraciones que redefinen `app.nap_publico` — antes solo conocía la `0020`, ahora también la `0023`).
> `renderer/src/perfil.test.ts`: 41/41; `tres-fronteras.test.ts` (el checkpoint que dependía de esta
> pieza) cerró en verde. Root (`npm run verificar`, esta corrida): **1518 tests, typecheck limpio, sin
> secretos**.
>
...
| ✅ | **1395 tests en verde** (+369 en el portal: 248 `node:test` + 121 Karma) + typecheck limpio en los 7 paquetes. Los de seguridad, contra Postgres real. |
