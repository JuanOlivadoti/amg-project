...
| **Tests** | **1716** — 1372 en el monorepo + 344 en el portal (237 `node:test` + 107 Karma). Los de seguridad, contra Postgres real. Cubre los bloques A (**A4 incluido**), B y **C entero**, el **bloque E completo** (entregas 1, 2 y 3), las **etapas 1-4 del bloque J** (Ideas) y la **etapa 1 del rediseño de la plantilla base** (tema claro, cabecera y portada) (venía de 1199 → … → 1659 → 1693 → 1712 → 1716). El portal, con `-- --con-portal` y Karma aparte. ✅ **La cifra del 2026-08-10 (tarde) SÍ es una corrida de `npm run verificar` en verde entero**, con el gate de paridad **re-capturado** tras medir que no se perdía nada (cero palabras, `href`, `id`, JSON-LD ni trazas en los diez casos). Sustituye a la suma por paquete que hubo que anotar unas horas antes, mientras ese trabajo estaba sin commitear. |
...
| ✅ | **1372 tests en verde** (+344 en el portal: 237 `node:test` + 107 Karma) + typecheck limpio en los 7 paquetes. Los de seguridad, contra Postgres real. |
