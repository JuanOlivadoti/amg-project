...
| **Migraciones** | **18 en `main`, 17 aplicadas en producción** (`0001`..`0012` + `0015`..`0019`), la `0018` y la `0019` el **2026-08-08**. ⏳ **La `0014` está PENDIENTE de desplegar** (`migrate:deploy -w db`, un paso de Juan): re-materializa `business_profile_publico`, así que hasta que corra el renderizador desplegado **no ve ni una foto ni un token de marca** aunque el código sí los sepa leer. La próxima libre es la **`0020`**: la `0013` sigue **reservada** para la pieza 3 del portal (Ideas). **El primer intento de la `0018` falló** (`must be able to SET ROLE "app_barrido"`) y se revirtió sola: en PGlite el rol que migra es superusuario y en Supabase alojado no, así que `alter … owner to` exigía dos permisos que nadie había concedido. Arreglado y con un test que aplica esa migración **como rol no-superusuario** — sin él, la clase entera volvería a escaparse |
