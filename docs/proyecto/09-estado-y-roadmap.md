...
| **Tests** | **1612** — 1268 en el monorepo + 344 en el portal (237 `node:test` + 107 Karma). Los de seguridad, contra Postgres real. Medido el **2026-08-09** con `npm run verificar` tras la 15ª review, los bloques A (**A4 incluido**), B y **C entero**, y el **bloque E completo** (entregas 1, 2 y 3, con el gate de paridad re-capturado al cerrar) (venía de 1199 → … → 1521 → 1541 → 1564 → 1612). El portal, con `-- --con-portal` y Karma aparte. |
...
| ✅ | **1268 tests en verde** (+344 en el portal: 237 `node:test` + 107 Karma) + typecheck limpio en los 7 paquetes. Los de seguridad, contra Postgres real. |
