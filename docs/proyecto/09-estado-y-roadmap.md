...
> vertical `correduria_seguros` la primera herramienta de día a día: un corredor sube un CSV/`.xlsx` o
> pega el link de un Google Sheet con cotizaciones de varias aseguradoras, un LLM (o un mock
> determinista) arma una comparativa, alguien la revisa —el gate central, impuesto por Postgres, no por
> un `if`— y recién entonces se puede imprimir o copiar el mail para el cliente final.
>
> **La doctrina central, "fallar en vez de inventar", se sostiene con constraints y tests que muerden,
> no con comentarios**: el parser de CSV (duplicado a propósito entre `api/` y `portal/`, atado por un
> test cruzado que importa el archivo del portal en runtime) lanza ante una comilla sin cerrar en vez de
> fusionar filas en silencio; el provider real rechaza la comparativa ENTERA ante una opción sin
> aseguradora, sin prima o con prima ≤ 0, o ante confianza que no sea `"alta"`; y el `.xlsx`/CSV se
> parsea siempre en el NAVEGADOR — el único texto que baja del servidor es el export de un Google Sheet
> compartido, por CORS, nunca un binario (misma doctrina que el PDF del 2026-08-07).
>
> **El costo real está acotado por tres capas**: `COMPARATIVAS_MODO` (`mock`/`openai`, default `mock`,
> falla cerrado ante un valor inválido), un preflight que aborta ANTES de llamar a OpenAI si la
> estimación supera `MAX_COSTO_USD`, y el modo mock marca el prefijo `[COMPARATIVA MOCK]` en el informe
> **y** en el mail (asunto y cuerpo) — el mail es el entregable que se copia con un botón y sale al
> cliente final sin pasar por la vista del informe, así que la marca tiene que estar en los dos.
>
> **Una ronda de fixes cerró lo que las revisiones por tarea habían dejado diferido a propósito**: la
> carrera de `revisar` (dos peticiones concurrentes ya no pueden pisarse: `and revisado_en is null` en
> el `where` del `update`, impuesto por Postgres) y los menores acumulados. **Manejando la app en un
> navegador real** (provider mock) encontré y arreglé un bug que ningún test veía: el `<input
> type="file">` nativo no se limpiaba visualmente al elegir un Google Sheet en su lugar.
>
> **La revisión final de rama (la única que mira todo el diff junto, no tarea por tarea) confirmó el
> diseño** —doctrina, gate, RLS/tenancy, ningún binario cruza al servidor, sin HTML crudo— **y pidió
> tres cambios antes del merge**, ninguno de diseño: clasificar `COMPARATIVAS_MODO` en el catálogo de
> credenciales (rompía `npm run verificar`), un test que confirme que un rol `cliente` no puede cerrar
> el gate de revisión, y esta misma actualización de documentación. **Pendiente de Juan, no bloqueante
> para el merge:** calibrar la proporción caracteres/token del preflight de gasto contra una corrida
> real de OpenAI (cuesta dinero). **Riesgo aceptado a mencionarle al mergear:** la librería de `.xlsx`
> del portal (`read-excel-file`) depende de una sola cuenta de npm en toda su cadena de dependencias.
>
> 🧭 **Nuevo (2026-09-10): el checklist de decisiones de Juan queda CERRADO, salvo dos números.** Las
> cinco que estaban abiertas se recorrieron una por una: `CACHE_TTL_MS` = **60 s**; el límite de
> dominios custom se **espera** con un disparador escrito (el tercer cliente con dominio propio);
> **OBS-04 se cierra en (a) — edita solo la agencia**, con **(c) desde el portal** nombrado como único
> camino de crecimiento y **(b) un seat en Storyblok** descartado; la **salida gestionada** toma forma
> de **pago único + cuota mensual** (los dos importes siguen pendientes de Juan); y el trámite de la
> **Business Profile API queda en pausa**, así que el módulo 3 sigue siendo una demo. **Con OBS-04
> cerrada, las 4 observaciones están cerradas y ADR-11 se puede reescribir.** Detalle en
> [`16-pendientes-juan.md`](16-pendientes-juan.md) y en § Roadmap.
>
> **De tomar esas decisiones salieron dos hallazgos.** (1) **El TTL no era lo que este documento
> creía**: el webhook de invalidación lo dispara sólo Storyblok, pero la home, `/menu`, la nav y el pie
> se sintetizan desde `business_profile` (Postgres) y se hornean en el HTML cacheado — un cambio hecho
> desde el portal **no invalida nada** y aparece sólo al vencer el TTL. (2) ⚠️ **Hay una trampa armada
> en producción**: `crearFuncionPollingResenas` está registrada incondicionalmente, así que el día que
> alguien pulse «Conectar Google» estando en `GOOGLE_REVIEWS_MODO=mock` se insertan **dos reseñas
> inventadas en la base real**, se dispara una **alerta de Telegram de verdad** y se gasta una **llamada
> real a OpenAI**. Acotado pero permanente, y como el trámite quedó en pausa **no caduca solo** — así
> que **se desarmó el mismo día** (ver la entrada de abajo).
>
> 🧭 **Nuevo (2026-09-10, mismo día): el guardarraíl de «Conectar Google» está IMPLEMENTADO.** La
> regla —`esProduccion && GOOGLE_REVIEWS_MODO === "mock"`— vive en `api/src/deps.ts`, el único archivo
> que toca `process.env`, y `esProduccion` sale de la sonda del SDK de Inngest ya existente (extraída a
> `esModoProduccion()` para no tener dos lecturas que diverjan), nunca de un `if` sobre variables de
> PaaS escrito a mano. `ApiDeps.conectarGoogleBloqueado` es **obligatorio sin default** —fuera de
> producción el valor correcto es `false`, así que cualquier default sería `false` y un cableado
> olvidado dejaría la trampa armada—, o sea que **es el typecheck** el que obliga a cada composition
> root a decidir. **Dos puntos de corte:** `POST …/google/conectar` antes de acuñar el `state`, y
> `GET …/google/callback` antes de toda escritura — el segundo porque **un `state` firmado antes del
> despliegue sigue vivo 10 minutos**, y hay un test que lo firma con la app sin guardarraíl y lo
> presenta a la app con guardarraíl. **409 y no 403** (el 403 está reservado a `42501`), y sin `codigo`
> —el criterio de `api/src/codigos.ts` siempre fue *si el portal ramifica*, y acá sólo muestra el
> mensaje; la nota de alcance de ese archivo se corrigió—. **`desconectar` NO se bloquea**, con un test
> que lo impone: bloquear el remedio dejaría a un cliente conectado por error sin salida. Y el portal
> ganó el `try/catch` que le faltaba: sin él el botón fallaba **en silencio**, el peor resultado
> posible para un guardarraíl. ⚠️ `INNGEST_DEV=1` ahora apaga **dos** cosas: la exigencia de
> `INNGEST_EVENT_KEY` y este guardarraíl (hay un test que lo fija, no solo el comentario).
>
> ⚠️ **Lo que el guardarraíl NO deshace: una fila que YA esté conectada.** Corta el camino de
> *conectar*, no el polling. Si existe un cliente con `google_refresh_token` en producción sigue
> sembrando reseñas falsas en cada ciclo — sin verificar, es la comprobación que queda pendiente de
> Juan. El remedio es `desconectar`, que NO se bloqueó a propósito.
>
> 🧭 **Nuevo (2026-09-11): ADR-11 REESCRITA — el offboarding ya tiene política, y le falta una sola
> pieza de código.** El ADR arrastraba desde 2026-07-14 el cartel de *"hay que reescribirlo antes de
> llevarlo a un contrato"*: estaba redactado sobre un frontend Next.js que ADR-16 eliminó, y
> «editable» significaba dos cosas hasta que OBS-04 se cerró. Ahora tiene una sección **«versión
> vigente»** —la que se lleva a un contrato— y el cuerpo viejo queda marcado como historia.
>
> Dice: **snapshot estático incluido**; **salida gestionada de pago** (pago único + cuota mensual,
> importes pendientes de Juan); y **«editable» = una capacidad que el cliente GANA en la baja**, nunca
> una que ya tenía, porque OBS-04 se cerró en (a). **La variante (b1) «el cliente lo hostea» se RETIRA
> de la oferta** — el renderizador es multi-tenant y lee de la base de AMG, así que quien se lleve su
> space se lleva el contenido y nada que lo dibuje. Se retira como *«hoy no se ofrece»*, **no** como
> *«no se puede»*: medido al escribirlo, `demo-server.ts` ya corre el renderizador entero contra
> **PGlite en memoria sembrado desde un JSON de perfil**, así que las piezas existen y lo que falta es
> empaquetado, documentación y soporte.
>
> **Lo que impide firmarlo hoy son dos cosas, y una es código:** los dos importes (Juan) y **verificar
> el snapshot estático como entregable** — se puede sacar de `renderStory()`, pero **nadie lo hizo
> nunca**, así que esa línea del contrato describe hoy una intención.
>
> **El orden de trabajo que seguía:** quedaba el tercer y último ítem del lote corto —**verificar el
> snapshot estático**— y después la pieza grande, **comparativas de seguros**. Las dos están hechas —
> ver la entrada del 2026-09-14 más abajo. El detalle, en
> [`15-plan-plataforma.md`](15-plan-plataforma.md) § El orden que recomiendo.
>
> 🧭 **Nuevo (2026-09-11): el snapshot estático EXISTE y está verificado — el lote corto queda cerrado
> entero.** `npm run snapshot -w renderer -- <dominio> <destino>`. Era la última pieza de código del
> Bloque H y la que impedía firmar ADR-11: hasta ayer el contrato prometía un entregable que **nadie
> había producido nunca**. **Ahora a ADR-11 le falta UNA sola cosa, y no es código: los dos importes
> de Juan.**
>
> **Las imágenes se descargan** (decisión del usuario), así que el entregable no depende de que siga
> viva ninguna cuenta de Storyblok — que es literalmente lo que la cláusula promete. Y renderiza con
> **el mismo código que sirve el sitio vivo**: la decisión «qué HTML le toca a cada slug» se **extrajo**
> del handler de `app.ts` a `renderer/src/pagina.ts` y ahora la comparten los dos. Copiar las reglas
> habría cumplido la letra del contrato y fallado a los seis meses, cuando una regla nueva se cambiara
> en un sitio y el snapshot se desincronizara **sin error y sin log**. Los 178 tests del renderizador
> siguieron en verde sin tocar ninguno.
>
> **Verificado en un navegador** —que es lo que esta promesa exige y ningún test ve—: 6 páginas, 10
> imágenes, 7 tipografías, **cero peticiones a hosts externos** y **cero JavaScript ejecutable** (los
> únicos `<script>` son el JSON-LD y el `research-trace` de las landings, los dos que el sitio vivo ya
> emite).
>
> **Dos hallazgos de la mutación que valen más que el comando.** Dos mutaciones **no hicieron caer
> nada**, y las dos destaparon tests que pasaban por el motivo equivocado: el de `og:image` (lo salvaba
> que el JSON-LD trae la misma URL, así que el día que una página lleve `og:image` sin `ld+json` la
> fuga habría aparecido en silencio) y el filtro del índice de la home, que **no lo fijaba ningún
> test, ni antes ni después** — el de paridad no lo ve, porque una mutación en el código *compartido*
> mueve los dos lados igual. Es la debilidad conocida de un test de paridad, ahora escrita. Los dos
> tests que faltaban, agregados.
>
> ⚠️ **Y una deuda vieja que reapareció: `brand.tema` no cruza tres de las cuatro fronteras, así que
> hoy ningún cliente puede tener modo oscuro.** No la descubrió esta etapa —**estaba declarada desde
> el 2026-08-10** en el `15`— pero el navegador del snapshot volvió a tropezar con ella, así que sube
> a «Deuda conocida» con dueño en vez de quedarse en una nota de una etapa cerrada. Lo que importa no
> es el hallazgo: es que se conocía y se dejó pasar un mes.
>
> 🧭 **Nuevo (2026-09-03): sub-proyecto 3 (publicar posts en blog externo) — IMPLEMENTADO Y MERGEADO
> A MAIN. Es el tercero y último de la iniciativa de generalización en tener código real: los tres
> quedan con código implementado.** Las 12
...
> dejarlo anotado.
>
> **La revisión externa de Codex, indisponible el 2026-09-03, se sustituyó por una revisión final de
> rama del agente `revisor`** (mismo diff, `890490e..815edcb`, 16 commits) — no es una ronda de Codex
> más y no cuenta en la contabilidad de `08-testing-calidad.md`, pero se le pidió el mismo nivel de
...
> causaba un 500 en `GET /clients`, ya resuelto en otra sesión). Re-verificado sobre el RESULTADO del
> merge, no solo sobre la rama: mismas cifras (1861/328/266), typecheck limpio, sin secretos,
> confirmado con `PWD-CHECK`/`HEAD-CHECK` contra `5a8b663`. Con esto, **los tres sub-proyectos de la
> iniciativa de generalización quedan con código implementado y en `main`.**
>
> **Actualización (2026-09-03, mismo día): las migraciones `0027`-`0031` YA ESTÁN desplegadas a
> producción** — el usuario las corrió (fuera de Claude Code, `npm run migrate:deploy` está denegado
> acá a propósito) tras el fix del bloqueo de la `0018`. Verificado consultando el registro real,
...
> sesión previa; 6-14 esta sesión) ejecutadas con `superpowers:subagent-driven-development` —
> implementador y revisor independiente por task, con fix rounds en 4 de ellas (7, 10, 11, 13),
> todas hallazgos Important o Critical reales, ninguno cosmético. `clients.vertical`
> (`'restauracion' | 'correduria_seguros'`, columna `not null` sin default, inmutable por trigger de
> Postgres — migraciones `0029`/`0030`, todavía SIN desplegar a producción a propósito) gobierna
> cuatro fronteras: la allowlist de `app.nap_publico`, la receta de render (`juegoDe(vertical)`
> reemplaza a `juegoDe(brand.plantilla)`), el slug/JSON-LD de la página de catálogo pública (`/menu`
> + `Menu` para restauración, `/polizas` + `ItemList` para seguros) y el tab/formulario del portal.
> El catálogo (menú o pólizas) reusa el mismo contenedor de datos (`business_profile.menu`/
> `menu_categorias`, tipos `MenuItem`/`MenuCategoria` sin renombrar — decisión de costo/beneficio del
> propio plan) — es un cambio de COMPORTAMIENTO por vertical, no un tipo nuevo.
>
> **Dos veces se aplicó defensa en profundidad, no solo ocultar controles en la UI:** `unPlato()`
> (`web-builder`, Task 8) y `platoDesdeFormulario()` (portal, Task 13) descartan video/alérgenos/
> etiquetas/nutrición para un cliente de seguros AUNQUE el objeto de entrada los traiga cargados —
> ocultar el campo del formulario no alcanza, porque un dato "dormido" de antes de que el cliente
> fuera de seguros (o cargado por otra vía) igual podría sobrevivir si la única defensa fuera visual.
> Los dos casos se probaron con un test que construye el dato CON esos campos poblados antes de
> llamar en modo seguros, no uno que ya los omite.
>
> **Cuatro hallazgos reales de las revisiones independientes, cerrados con fix rounds:** un
> comentario de cabecera en `web-builder/src/render/html.ts` que afirmaba que la firma de las cuatro
> funciones de render "no cambia" — falso desde el mismo commit que le agregó `vertical` (Task 7);
> `PerfilSeguros` no estaba re-exportado desde `web-builder/src/index.ts`, así que `npm test` (que
> corre con `tsx`, sin chequeo de tipos) no lo detectaba pero `npm run typecheck` sí (Task 10);
> `GET /clients/:id/seguros` devolvía 200 siempre, incluso para un cliente ajeno o inexistente,
> inconsistente con `GET /clients/:id/menu` que sí devuelve 404 ahí (Task 11); y la primera pasada
> del tab de catálogo por vertical solo condicionó el `<h1>` invisible (`sr-only`), dejando visible
> "Platos"/"Agregar plato"/"Plato nuevo" sin condicionar en dos pantallas del portal — el `<h1>` real
> de `cliente-menu-detalle.ts`, el más visible de los cuatro, decía literalmente "Plato nuevo" para
> una póliza nueva (Task 13).
>
> Verificado en un navegador real, con datos reales, de punta a punta: alta de un cliente
> `correduria_seguros` desde el portal, tab "Pólizas y coberturas", editor de catálogo sin controles
> de restauración, quinto card de perfil (licencia/experiencia/red, `ClienteSegurosCardComponent`,
> Task 14 — el primer mecanismo de lectura/escritura de `business_profile` fuera de `menu` que existe
> en el portal) cargando/editando/guardando contra la API real, sin regresión para un cliente de
> restauración existente, en claro y en oscuro. Del lado público: un renderer propio (harness
> desechable, nunca comiteado) sirvió `/polizas` con JSON-LD `ItemList` (no `Menu`), `/menu` dando
> 404, el nav con la etiqueta correcta, y la sección de contacto mostrando licencia/experiencia/red —
> sin ningún "permission denied", confirmando que los grants de `app_render` alcanzan de verdad, no
> solo bajo PGlite superusuario.
>
> `npm run verificar --con-portal`: **1779 tests del monorepo** (sube de 1725) + **304 `node:test`**
> y **238 Karma** del portal (sube de 213/218), todos en verde — corrida con el directorio y el HEAD
> confirmados en el propio log (`PWD-CHECK`/`HEAD-CHECK`) después de que dos corridas previas, sin
> ese blindaje, midieran por error el checkout `main` en vez del worktree sin dar ningún síntoma
> visible (ver `progress/current.md` § Callejones sin salida para el detalle — el cwd del bash tool
> de la sesión se resetea solo tras ciertas notificaciones en background). **Migraciones
...
> código real.** Las 12 tasks del [plan](../superpowers/plans/2026-08-26-desacoplar-kr-web.md)
> ejecutadas con `superpowers:subagent-driven-development` (implementador + revisor independiente por
> task, más fix rounds donde hizo falta). Partió `workflowResearch` en dos funciones Inngest: el
> research (persiste y deja `pending_approval`, sin esperar nada) y `workflowDecision` (disparada por
> `research/aprobado`, que ya no lleva el destino — solo `tenantId`+`decisionId` — y relee la
> autoridad real de `kr_run_decisiones` bajo RLS antes de bifurcar). La tabla nueva
> `kr_run_decisiones` (migración `0027`) registra el destino elegido al aprobar (`crear_web` /
> `solo_informe` / `crear_posts` rechazado con 501 hasta el sub-proyecto 3) y hace **retomable** un
> run cerrado en `solo_informe` hacia `crear_web` más adelante — con la carrera de doble aprobación
> cerrada por un índice único parcial en Postgres (`kr_run_decisiones_una_pendiente`), no por lógica
> de aplicación. `RunSinWorkflowError`/`approveRun`/`RUN_SIN_WORKFLOW` (el mecanismo viejo,
> aprobación-y-publicación-inline con compuerta embebida) quedan retirados de punta a punta —
> `db/`, `orchestrator/`, `api/` y `portal/`.
>
> Un hallazgo real que ninguna de las dos rondas de Codex sobre el plan había visto, encontrado recién
> al implementar: el `WHERE` de `registrarDecision` (que exige el run en `pending_approval` para
> calificar una decisión) dejaba una decisión `pendiente` bloqueada PARA SIEMPRE si `emisor.send()`
> fallaba después de insertarla — el índice único parcial que cierra la carrera de doble aprobación
> impedía cualquier reintento. Cerrado con `PgStore.compensarAprobacionFallida`: revierte
> `kr_runs.status` a `pending_approval` SOLO si no hay ninguna decisión `completada` previa (protege
> el camino retomable), permitiendo que una segunda aprobación normal, después del fallo, sí califique.
> Verificado con test dedicado (guard retomable) y por mutación.
>
> **Tropiezo real de esta etapa, documentado porque puede repetirse:** esta implementación se hizo dos
> veces en paralelo, sin que ninguna de las dos sesiones supiera de la otra — una en el worktree
> `desacoplar-kr-web` (esta sesión) y otra, ya terminada un día antes, en `sub2-desacoplar-kr-web`. Se
> detectó al notar que `EnterWorktree` había ramificado desde `origin/main` (9 commits detrás del
> `main` local) en vez de desde el `main` local real, lo que obligó a investigar el estado de otros
> worktrees — y ahí apareció el segundo, ya con las Tasks 1-11 completas y revisadas con el mismo
> rigor. Comparados ambos (mismo tamaño de diff, mismos hallazgos independientes —
> `compensarAprobacionFallida` incluido—, éste con más cobertura de test en un par de puntos), se
> adoptó `sub2-desacoplar-kr-web` y se descartó el propio: no se perdió nada real porque ninguno de
> los dos había llegado a `main`. La Task 11 de ese worktree tenía un fix de autorevisión
> (`destinoElegido` no se reseteaba al navegar entre runs del brief) sin comitear y sin pasar por el
> revisor independiente — cerrado antes de continuar. **Lección: un worktree nuevo hereda de
> `origin/<default>`, no del `main` local — verificar `git log -1 origin/main` vs `main` antes de
> asumir que un worktree recién creado tiene el estado completo, y revisar `git worktree list` al
> arrancar una tarea larga para descartar trabajo paralelo ya en curso.**
>
> `npm run verificar --con-portal`: **1725 tests del monorepo** + **300 `node:test`** y **213 Karma**
> en el portal, todos en verde. Sin secretos, typecheck limpio en los 7 paquetes + `scripts/` +
> portal. **Migración `0027_kr_run_decisiones.sql` aplicada solo en PGlite — todavía NO desplegada a
> producción** (pendiente de `migrate:deploy`, a coordinar con el orden de implementación de los otros
> dos sub-proyectos, que comparten archivos con éste).
>
> **Hueco explícito, documentado y no oculto:** el camino POSITIVO del botón "Construir la web ahora"
> (retomar `solo_informe` completado hacia `crear_web`) no se verificó end-to-end contra un navegador
> real — requiere el orquestador y la API compartiendo el mismo Postgres real (con credenciales de
> `amg_orquestador`), y el entorno de desarrollo local corre cada uno sobre su propia PGlite en
> memoria. Cubierto en cambio por 7 tests de Karma contra un `Brief` con `ultimaDecision` mockeada. El
> resto del flujo (selector de destino, aprobar página, aprobar run con `solo_informe`, el 409
> `TRANSICION_INVALIDA` real al reintentar sobre una decisión ya `pendiente`) sí se verificó en un
> navegador real contra la API real.
>
> **Qué sigue:** implementar el sub-proyecto 1 (multi-vertical de clientes) o el 3 (publicar posts en
> blog externo), en cualquier orden entre ellos pero **siempre en serie** — comparten archivos
> (`db/src/store.ts`, `api/src/app.ts`, `orchestrator/src/workflow.ts` y sus tests) con éste y entre
> sí. Antes de arrancar cualquiera de los dos: correr `migrate:deploy` para la `0027` y confirmar
> `ls db/migrations` para el número real de la migración que le toque al que arranque primero.
>
...
> arranca la implementación. **Los sub-proyectos 1 y 2 ya tienen spec+plan completos**, cada uno con
...
> contrato compartido) en vez de la skill genérica — contrato fijado por la sesión principal antes de
> delegar. Migración `0025` (dos columnas + dos funciones `security definer`, sin política nueva),
> comando compuesto igual que `POST /runs/:id/approve` (ADR-18: la fila manda bajo RLS, el evento
> `resenas/respuesta.solicitada` solo lleva el `id` de la reseña), y el botón de tres estados en el
> portal. Sin reintento automático — el staff reintenta con el mismo botón. `revisor` verificó por
> mutación de primera mano los dos guardarraíles centrales, veredicto **APROBADO**
> (`progress/informes/revision-publicar-respuesta.md`). Detalle en
> [15-plan-plataforma.md § Bloque F](15-plan-plataforma.md#bloque-f--módulo-3-respondedor-de-reseñas-de-google).
>
> 🧭 **Nuevo (2026-08-24): Bloque F fase 2, tercera pieza — alertas por Telegram para reseñas 1-3★,
> COMPLETA. Cierra RF-018 del PRD.** Investigación previa a escribir código: WhatsApp Business exige
> verificación de negocio ante Meta y plantillas pre-aprobadas para cualquier mensaje que AMG inicie;
> Telegram no tiene ese gatekeeper (bot por `@BotFather`, gratis, sin aprobación) y el RF-018 pide la
> alerta **"al CM"** — personal interno de AMG, no al cliente —, así que la fricción de WhatsApp no
> compraba nada. [Plan de 3 tasks](../superpowers/plans/2026-08-23-alertas-telegram.md), revisado por
> **Codex antes de escribir código** (veredicto original NECESITA REDISEÑO, 8 hallazgos — 2 hubieran
> bloqueado el polling o fallado en runtime, el resto validaciones/tests/config, todos incorporados al
> plan antes de dispatchear la Task 1), y ejecutado con los agentes de área (`datos` → `pipeline` →
> `front`, en serie): migración `0026` (vinculación self-service con TTL de 10 min generado por
> Postgres, trigger `membresias_guardia_telegram` como backstop de la segunda política UPDATE
> permisiva sobre `memberships`, rol cross-tenant `app_telegram`) y retry automático vía
> `resenas_google.alerta_telegram_enviada_en` — sin cola ni botón: si el envío falla, el próximo ciclo
> de polling (30 min) reintenta solo. `revisor` pidió un cambio en la Task 1 (`GET /members` no
...
> ya está implementado de punta a punta (`renderer/src/deps.ts`: `leerConfig()` lo lee y lo valida,
> `crearDeps()` lo pasa a `CacheRender`) — solo le faltaba test, y sin test un default de producción
> que decide la propagación de un SLA es "una decisión sin dueño". `renderer/src/deps.test.ts` (10
> tests, nuevo) lo cierra, verificado por mutación. Con el TTL como único mecanismo (sin depender de
> a qué instancia le llegó el webhook), la peor propagación cruzando toda la flota queda acotada por
> `CACHE_TTL_MS`, sin importar cuántas instancias corran — lo único que falta es fijar el número en
> Railway según lo que pida el SLA (default 5 min si no se toca). Se evaluó cache compartida (Redis,
> invalidación casi en tiempo real) y se descartó: el SLA en curso pide acotar la propagación, no
> tiempo real. Los otros dos ítems del Bloque G (CDN en el borde, límite de dominios custom de
> Railway) son decisiones de despliegue/plan, no ingeniería, y quedan sin tocar. Detalle en
> [15-plan-plataforma.md § Bloque G](15-plan-plataforma.md#bloque-g--lo-que-adr-19-dejó-a-medias).
>
> 🧭 **Nuevo (2026-08-22): Bloque H — el enlace de preview del Visual Editor ya no se emite a mano.**
> `npm run preview:firmar -w renderer -- <dominio> [minutos]` (`renderer/src/cli/firmar-preview.ts`,
> 5 tests, verificado por mutación) envuelve `firmarPreview()` —hecha y probada desde ADR-19, pero
> hasta ahora usada con un script fuera del repo, sin test ni historia en git— en un comando
> versionado. `PREVIEW_SECRET` sigue sin viajar por `env:sync` a propósito: es config de Railway,
> igual que `DATABASE_URL_RENDER`/`STORYBLOK_WEBHOOK_SECRET` — el renderizador es la única pieza
> expuesta a internet anónimo (ADR-19), sus credenciales de producción no tienen por qué estar en
> ninguna máquina local. Los otros tres ítems del Bloque G/H (CDN, invalidación multi-instancia,
> OBS-04, precio de la salida gestionada) quedan sin tocar: son infraestructura sin urgencia hoy o
> decisiones de negocio que le tocan a Juan. Detalle en
> [15-plan-plataforma.md § Bloque H](15-plan-plataforma.md#bloque-h--offboarding-adr-11-y-obs-04).
>
> 🧭 **Nuevo (2026-08-22): Bloque D — corrida real de calibración contra DataForSEO en producción.**
> `$0.2124` (23 keywords, 3 páginas), dataset persistido en `datasets/keywords.json`. Verificó
> `TIPOS_MAP_PACK` contra la API real (3/3 cabezas, 1 `is_local` corregido) y recalibró
> `lib/budget.ts` con un dato real — el default anterior subestimaba el enriquecimiento ~60%, ya
> corregido con test. `VOLUMEN_PERCENTIL_TOPE`/`PESO_CONFIANZA_ORDEN` **siguen sin recalibrar a
> propósito**: solo 4 de 23 keywords trajeron volumen conocido, insuficiente para medir nada sin
> inventar precisión. Detalle en [15-plan-plataforma.md § Bloque D](15-plan-plataforma.md#bloque-d--calibrar-el-research-kr-1).
> Un intento previo corrió sin querer contra sandbox (un `env:sync` de por medio revirtió
> `DATAFORSEO_BASE_URL`) — sin costo, descartado antes de repetirlo bien.
>
> 🧭 **Nuevo (2026-08-21): Bloque F fase 2 (primera pieza) — borrador de respuesta con IA para
> reseñas de Google 4-5★, COMPLETA y mergeada a `main`.** Spec ([diseño](../superpowers/specs/2026-08-18-borrador-ia-resenas-google-design.md),
> 8 hallazgos de Codex aplicados) y [plan de 7 tasks](../superpowers/plans/2026-08-18-borrador-ia-resenas-google.md)
> ejecutados con `superpowers:subagent-driven-development` sobre un worktree. Migración `0024`
> (columnas `borrador_respuesta`/`borrador_generado_en` + `app.guardar_borrador_resena`, `security
> definer`, el `where puntuacion between 4 and 5` lo impone Postgres, no el llamador),
> `BorradorProvider` mock/OpenAI (`BORRADOR_RESENAS_MODO`), `pollearResenas` generando el borrador
> dentro del mismo ciclo de polling, y el tab del portal con textarea editable. Detalle completo,
> incluidos los dos bugs reales encontrados (uno por verificación manual, otro por la re-revisión de
> los fixes), en [15-plan-plataforma.md § Bloque F](15-plan-plataforma.md#bloque-f--módulo-3-respondedor-de-reseñas-de-google).
> Migración `0024` desplegada a producción el **2026-08-22**.
>
> `npm run verificar --con-portal`: **1599 tests del monorepo** + **298 `node:test`** y **192 Karma**
> en el portal, todos en verde. Verificado en el navegador (conectar Google, los tres estados de
> reseña, editar/guardar con persistencia confirmada tras reload, consola limpia).
>
...
| | |
|---|---|
| **Paquetes** | 8 workspaces (`contrato`, `db`, `kr-service`, `web-builder`, `orchestrator`, `api`, `renderer`, `mcp-server`) + `portal/` (Angular, fuera del monorepo a propósito). `mcp-server` (2026-09-15) no tiene agente de área propio — lo lleva la sesión principal (`AGENTS.md`) |
| **Tests** | **2755** — 2092 en el monorepo (8 paquetes + `scripts/`, confirmado con `bash ./scripts/verificar.sh --con-portal` real tras el merge, exit 0) + **663 en el portal** (357 `node:test` + 306 Karma — sin cambios del lado de `mcp-server/`, que no toca `portal/`; la cifra de portal es la de `comparativas-seguros`, que sí lo tocó). Subió **+54 con el MCP local para Claude Desktop** (2026-09-15, paquete nuevo `mcp-server/`, implementado en un worktree aislado mientras otra sesión trabajaba `comparativas-seguros` en paralelo, sin cruce de archivos): `session.ts` (la sesión en disco, atómica — 11), `gotrue.ts` (login/refresh contra Supabase Auth, con test que fija la forma exacta del request — 7), `api-client.ts` (el cliente HTTP hacia `api/`, refresh automático y la tabla de errores del spec §7 — 14, incluidos los 3 que se sumaron al fijar `MARGEN_REFRESH_MS` por valor y por comportamiento en el borde tras el hallazgo del `revisor`), `validacion.ts` + las tools de clientes y runs (la barrera de UUID y el enum cerrado de `destino`, mutación confirmada — 23), `formato.ts` (2) y `server.ts` (el registro de las seis tools contra el SDK real — 1). Revisado por `revisor`: CAMBIOS_PEDIDOS (1 bloqueante documental — esta misma actualización — + 1 menor ya corregido + 1 menor de entorno, ajeno al paquete). Falta la prueba manual contra Claude Desktop real (spec §10, no la puede hacer una sesión de Claude Code sola). Antes, subió con **comparativas de seguros** (2026-09-12/14, plan de 9 tareas + ronda de fixes pre-merge + fix de la review final): +91 en el monorepo (`db`: migración `0033`, tabla `comparativas_seguros` con RLS y el gate de revisión, `PgComparativasSeguros` — crear/obtener/listar/marcarRevisada, con el test de la carrera de `revisar` y su gemelo de que el rol `cliente` no puede cerrar el gate; `api`: parser de CSV con sus topes, descarga del CSV-export de un Google Sheet compartido, el contrato de provider de IA + mock determinista + provider real de OpenAI con preflight de gasto, los cuatro endpoints, los dos topes de longitud movidos ANTES de llamar al provider) + 25 `node:test` del portal (parser de CSV duplicado a propósito con test cruzado que importa el archivo del portal por ruta en runtime, conversión de `.xlsx`/CSV en el navegador con `read-excel-file`, `markdown-render.ts` para el botón "Copiar mail") + 28 Karma (pantalla de carga, resultado con el gate y el informe imprimible, historial, el tab de vertical, el reseteo del `<input type="file">` nativo en el XOR encontrado manejando la app en un navegador). Verificado con `npm run verificar --con-portal` en verde entero (exit 0) y Karma aparte — la primera corrida completa de la rama, después de que la review final (la única que mira el diff entero, con el modelo más capaz) pidiera clasificar `COMPARATIVAS_MODO` en el catálogo de credenciales y el test del gate para el rol `cliente`. Antes, subió con el **guardarraíl de «Conectar Google» en modo mock** (2026-09-10): +7 en el monorepo — 3 en `api/src/app.test.ts` (el 409 de `conectar` sin acuñar `state`, el corte del callback ante un `state` firmado ANTES del despliegue leyendo la fila para probar que no se escribió, y el que impide bloquear `desconectar`) y 4 en `api/src/deps.test.ts` (la regla `esProduccion && mock`, con sus tres casos y el de `NODE_ENV` que impide reimplementar la heurística del SDK) — más +1 Karma en `cliente-resenas.spec.ts` (el `try/catch` que le faltaba a `conectar()`: sin él el 409 se rechazaba sin que nadie lo mirara y el botón no hacía nada visible, verificado por mutación). **+47 con el snapshot estático de salida** (2026-09-11/12, `renderer`: 178 → 225; los últimos 6 salieron de la revisión del `revisor`, que mutó `PLAZO_DESCARGA_MS`, `MAX_REDIRECCIONES` y el nombre del informe y **no hizo caer ni un test** — los que había elegían el parámetro en vez de fijar el de producción, y el del informe se comparaba consigo mismo; se sumó además la precedencia de clase `contenido` > `social`, extraída a `referenciasDe()` para poder ejercitarla — 36 del motor y 5 del CLI con subprocesos reales; incluidos los dos que se agregaron *después* de que sus mutaciones no hicieran caer nada: el escaneo de `og:image` por emisor y el filtro del índice de la home, que no lo fijaba ningún test). Antes, **+2 al corregir la revisión del `revisor`** (2026-09-11): el test que ata el texto del 409 a `getGoogleOAuthProvider` —el mensaje prescribía `GOOGLE_REVIEWS_MODO=live`, que con el provider sin implementar **tira la API al arrancar**— y el que fija que `INNGEST_DEV=1` apaga el guardarraíl, la única forma de desactivarlo en producción y hasta entonces sostenida solo por un comentario. Antes, subió con el bloque **C-1** del plan (la marca de intento de publicación, 2026-09-04 — el dry-run deja de ser el único modo sin rastro nuestro): +15 en el monorepo (`db`: tabla `kr_publicacion_intentos` [migración `0032`] + `PgStore.registrarIntentoPublicacion`, 12 tests estructurales/RLS/grants/checks; `orchestrator`: `Deps.publicar` devuelve `{modo, resultados}`, el paso `publicar` de `workflowDecision` registra el intento SIEMPRE — dry-run puro, live completo, mezcla parcial —, 3 tests). Implementado en paralelo por `datos` y `pipeline` sobre un contrato fijado por la sesión principal; C-2 (si el barrido debería cancelar el workflow) se cerró como decisión documentada, sin código — ver `docs/proyecto/15-plan-plataforma.md` § C-1/C-2. Revisado por `revisor`: CAMBIOS_PEDIDOS por un hallazgo documental (esta misma actualización, más dos citas de línea a recalcular en el plan) — cero hallazgos de código. Antes, subió con el editor de contenido del Bloque E (`bienvenida`/`destacados`/`testimonios`, 2026-09-04, sexto card de `/clientes/:id/perfil`, cierra el Bloque E): +15 en el monorepo (`db`: `obtenerContenido`/`actualizarContenido`; `api`: los dos endpoints `/clients/:id/contenido`) + 15 en el portal (4 `node:test` en `api-core.test.ts`, 9 Karma en `cliente-contenido-card.spec.ts` nuevo, 2 en `cliente-perfil.spec.ts`). Implementado en paralelo por los agentes `datos`/`front` sobre un contrato fijado por la sesión principal, revisado por `revisor` (CAMBIOS_PEDIDOS por un hallazgo documental — esta misma actualización — sin hallazgos de código). Antes, subió con el sub-proyecto 3 de la iniciativa de generalización (publicar posts en blog externo, 2026-09-03, 12 tasks + 1 fix Critical en la Task 8): +81 en el monorepo (`db`: `sanitizarHtml`, los 7 métodos nuevos de `PgStore` para posts, RLS/grants del rol `app_posts`; `orchestrator`: `PostProvider`/`BlogPublisher` mock+OpenAI, `workflowDecision` rama `crear_posts`, `publicarPost`) + 50 en el portal (`posts-estado.ts`/`html-a-texto.ts` puros, la pantalla `pages/posts/` con su máquina de 4 estados y el botón Copiar). Verificado sobre el RESULTADO del merge a `main` (commit `5a8b663`), no solo sobre la rama — ver la entrada del resumen ejecutivo de arriba. Antes, subió con la 16ª review externa (Codex, 2026-08-31, fix round de código sobre el sub-proyecto 1 ya cerrado): +1 en `web-builder` (`ItemList.position` global, no por categoría, en `json-ld.ts`) + 2 Karma en el portal (la carrera de `idVigente` en `cliente-seguros-card.ts`: cambiar de cliente en edición o con un PATCH pendiente ya no pisa el cliente nuevo) — detalle y clasificación de los 2 hallazgos en [08-testing-calidad.md § Tanda 22](08-testing-calidad.md#revisiones-externas-codex--qué-encontraron-y-qué-se-corrigió). Antes, subió con el sub-proyecto 1 de la iniciativa de generalización (multi-vertical de clientes, 2026-08-30, Tasks 6-14 de esta sesión sobre las 1-5 previas): +54 en el monorepo (`db`: grants/allowlist de `vertical` y `obtenerPerfilSeguros`/`actualizarPerfilSeguros`; `web-builder`: `Vertical`/`PerfilSeguros`, `juegoDe(vertical)`, el juego `SEGUROS`, la defensa en profundidad de `cartaCategorias`; `orchestrator`+`renderer`: propagación de `vertical` a los 4 puntos de render de producción, `perfilValido`; `api`: `POST /clients` exige `vertical`, `GET`/`PATCH /clients/:id/seguros`) + 34 en el portal (10 `node:test` en `api-core.test.ts`/fixtures, 20 Karma nuevos entre `cliente-crear`/`cliente-ficha`/`cliente-menu`/`cliente-menu-detalle`/`cliente-perfil`/`cliente-seguros-card`, 4 en fixtures compartidos). Verificado con el directorio y el HEAD confirmados en el propio log de la corrida — ver la entrada del resumen ejecutivo de arriba y `progress/current.md` § Callejones sin salida. Antes, subió con el sub-proyecto 2 de la iniciativa de generalización (desacoplar keyword research de creación de webs, 2026-08-28, 12 tasks): +25 en el monorepo (`db`: `kr_run_decisiones` + `registrarDecision`/`cerrarDecision`/`getDecision`/`getUltimaDecision`/`getRunConUltimaDecision`/`compensarAprobacionFallida`, con sus tests de RLS/grants bajo roles reales y verificación por mutación del índice único parcial; `orchestrator`: `workflowDecision` y `crearFuncionDecision`, la migración completa de `workflow.test.ts`; `api`: `POST /runs/:id/approve` con `{destino}` y su transacción compensatoria si falla el emisor) + 11 en el portal (el selector de destino, `puedeRetomarUI`, el fix de `destinoElegido` al navegar entre runs). Detalle completo en el ledger de la sesión (`.superpowers/sdd/2026-08-26-desacoplar-kr-web/progress.md`, no versionado) y en la entrada del resumen ejecutivo de arriba. Antes, subió con el Bloque F fase 2, tercera pieza (alertas por Telegram, 2026-08-24): +61 en el monorepo (`db`/`api`/`orchestrator` — capa de acceso, endpoints de auto-servicio, provider mock/live y las dos funciones de Inngest) + 8 en el portal (+3 `api-core.test.ts`, +5 `usuario-perfil.spec.ts` Karma, la tarjeta "Vincular Telegram") — detalle en `progress/informes/{datos,pipeline,front}-task*-alertas-telegram.md` y sus tres revisiones. Antes, subió con el Bloque F fase 2, segunda pieza (publicar la respuesta a Google, 2026-08-23): +11 en `db` (migración `0025`, `resenas.test.ts`/`store.test.ts`), +5 en `api/src/app.test.ts` (el `PATCH` con `{"publicar": true}`), +7 en `orchestrator` (`provider.test.ts`/`functions.test.ts`, la función de Inngest nueva), +4 en `cliente-resenas.spec.ts` (Karma, el botón de tres estados) — verificado por `revisor` con mutación propia sobre los dos guardarraíles centrales, no solo los informes de los agentes de área. Antes, subió con el Bloque G (2026-08-23): +10 en `renderer/src/deps.test.ts` (`leerConfig()` no tenía NINGÚN test — cubre `CACHE_TTL_MS`, que dejó de ser una red de seguridad y pasó a ser el mecanismo de invalidación multi-instancia). Antes, subió con el Bloque H (2026-08-22): +5 en `renderer/src/cli/firmar-preview.test.ts` (el CLI que envuelve `firmarPreview()`, subproceso real). Antes, subió con el Bloque D (calibración real, 2026-08-22): +2 en `kr-service/src/lib/budget.test.ts` (`DEFAULT_ESTIMATES` recalibrado, fijado por test). Antes, subió con el Bloque F fase 2 — primera pieza (borrador de respuesta con IA, 2026-08-21): migración `0024` y su store en `db`, `BorradorProvider` mock/OpenAI y el cableado en `pollearResenas`/`config`/`app.ts` en `orchestrator`, el PATCH del borrador en `api`, y el tab editable en el portal — incluidos los guardarraíles nuevos que encontraron dos bugs reales (`listarResenas` no devolvía `borrador_respuesta`; `OPENAI_MODEL` caía en `""` en vez de su default cuando `env:sync` la sincroniza sin declarar). Antes, subió con el editor de carta en el portal (2026-08-17): +6 en `web-builder/src/contract.test.ts` (el Zod exportado + `menuPatchSchema`), +5 en `db/src/clientes.test.ts` (`obtenerMenu`/`actualizarMenu`), +7 en `api/src/app.test.ts` (los dos endpoints + el test de equivalencia) — 18 en el monorepo —, y en el portal +3 en `api-core.test.ts`, +2 en `menu-taxonomia.spec.ts`, +10 en `cliente-menu.spec.ts` y +10 en `cliente-menu-detalle.spec.ts` (Karma) — 25 ahí. Antes, subió con el menú digital enriquecido (video, alérgenos, etiquetas, nutrición, comensales — 2026-08-16): +5 en `web-builder/src/contract.test.ts` (Zod), +10 en `web-builder/src/render/videos.test.ts` (seguridad del video), +6 en `db/src/menu-enriquecido-publico.test.ts` (allowlist de Postgres, migración `0023`), +6 en `renderer/src/perfil.test.ts`, +17 en `web-builder/src/render/piezas/piezas-foto.test.ts` (render de la carta, incluidos los del presupuesto de video y del tope de alérgenos/etiquetas que agregó la revisión final de rama). Antes, subió con el Bloque F (reseñas de Google, fase 1: monitoreo + alerta, ocho tasks + el fix del callback OAuth firmado, 2026-08-15) — tests nuevos de RLS/credenciales en `db`, del `state` firmado y la conexión OAuth en `api`, del polling en `orchestrator`, y de `api-core.ts`/`cliente-resenas.spec.ts` en el portal. Subió 25 el **2026-08-11** con la navegación cliente-céntrica, y el saldo esconde un movimiento mayor: se **borraron** los specs de `/clientes/:id/ver` y de su mock al retirar la pantalla, y entraron los de los cuatro tabs, la conciliación cliente↔run, el destino post-login y **dos barridos nuevos** (`marca-activa`, que caza la clase de `routerLinkActive` que pierde la cascada; y `arbol-encabezados`, que impone contenedor-sin-`h1` / hoja-con-`h1`). Los dos barridos **descubren** las plantillas en vez de enumerarlas, así que cubren las pantallas que se agreguen. Los de seguridad, contra Postgres real. Cubre los bloques A (**A4 incluido**), B y **C entero**, el **bloque E completo** (entregas 1, 2 y 3), las **etapas 1-4 del bloque J** (Ideas) y el **bloque K entero** — el rediseño de la plantilla base, sus tres etapas (venía de 1199 → … → 1712 → 1716 → 1731 → 1739). El portal, con `-- --con-portal` y Karma aparte. ✅ **La cifra del 2026-08-10 (noche) es una corrida de `npm run verificar` en verde entero**, con el gate de paridad **re-capturado** tras medir que no se perdía nada (cero palabras, `href`, `id`, JSON-LD ni trazas en los diez casos) y con el sitio **manejado en un navegador** a 1440 y a 390. |
| **Migraciones** | **33 en `main`.** La `0033_comparativas_seguros.sql` (tabla `comparativas_seguros`, RLS y el gate de revisión) se mergeó a `main` el 2026-09-15 junto con el resto de `comparativas-seguros` — **falta confirmar si ya se desplegó a producción** con `npm run migrate:deploy -w db` (fuera de Claude Code); hasta entonces la tabla no existe ahí. La `0032_intento_publicacion.sql` (tabla `kr_publicacion_intentos`, bloque C-1) la corrió el usuario el **2026-09-05** (`npm run migrate:deploy -w db`, fuera de Claude Code): `+ 0032_intento_publicacion.sql` / `✔ Aplicadas 1 migración(es)`, era la única pendiente. `0027_kr_run_decisiones.sql`/`0028_client_row_archived_at.sql` (sub-proyecto 2), `0029_clientes_vertical.sql`/`0030_nap_publico_vertical.sql` (sub-proyecto 1) y `0031_posts_blog_externo.sql` (sub-proyecto 3) las corrió el usuario el **2026-09-03** (fuera de Claude Code — `npm run migrate:deploy` está denegado acá a propósito), mismo lote, `aplicada_en` **14:56:58 UTC** las cinco — verificado consultando `app.migraciones_aplicadas` en producción vía MCP de Supabase (`execute_sql`; `list_migrations` de esa misma integración mira el sistema de migraciones de la CLI de Supabase, una tabla distinta que este proyecto no usa, y por eso da vacío — no confundir las dos). **El bloqueo de Windows que había represado la `0021`-`0023` (ver más abajo) reapareció el 2026-09-03 con la `0018`**: un commit del 2026-08-27 (`1a14d3c`) le tocó solo el comentario a una migración YA aplicada, cambiando su checksum sin tocar SQL — `migrate:deploy` lo vio como "su contenido CAMBIÓ" y abortó antes de mirar ninguna pendiente, dejando `GET /clients` en 500 en producción (columnas de `vertical`/`blog_externo_*` que el código ya esperaba, sin la migración aplicada). Revertido el comentario a su texto tal como quedó aplicado (commit `3aa68cf`) — mismo tipo de bug que el de CRLF/LF de más abajo, esta vez por editar un comentario en vez de un `\r\n`: **la lección que falta imponer con una constraint, no solo con disciplina, es que ningún archivo de `db/migrations/` ya aplicado se vuelva a tocar, ni el comentario.** Antes, al día desde el **2026-08-24**, con la `0026` — Bloque F fase 2, tercera pieza: vinculación de Telegram + retry de alertas. Las `0021`/`0022` (Bloque F: tabla `resenas_google` + columnas de conexión en `clients`, y el rol cross-tenant `app_resenas` del polling, 2026-08-15) y la `0023` (menú digital enriquecido: `video`/`alergenos`/`etiquetas`/`nutricion`/`comensales` en `app.nap_publico`, 2026-08-16) quedaron represadas por un bug real de `migrate:deploy` en Windows: `checksumDe()` (`db/src/deploy.ts`) hasheaba el contenido crudo del archivo, y `core.autocrlf=true` (el default de git ahí) escribe `db/migrations/` en disco con CRLF mientras el checksum registrado en Supabase se calculó en LF — incluso sin que nadie hubiera editado una línea, el runner veía "la `0001` cambió después de aplicada" y abortaba **antes** de mirar ninguna migración pendiente. Confirmado leyendo el registro real: el checksum de producción coincidía EXACTO con el commit `bf3d1f7` de `0001_init.sql` normalizado a LF. Arreglado normalizando `\r\n` a `\n` antes de hashear; verificado por mutación. Corrido `migrate:deploy` de nuevo, las tres se aplicaron en el mismo lote. La `0013` (Ideas) y la `0020` (las tres secciones de plantilla) se aplicaron el **2026-08-10 por la noche** con `npm run migrate:deploy -w db`, y con eso **producción fue al día por primera vez desde el 2026-08-08** (hasta la `0020`). Las dos corrieron contra una base que ya tenía la `0014`-`0019`, que es exactamente el escenario "al final" que modela el test de orden de la `0013`; la `0020` corrió después de la `0014`, que era su única dependencia real. Corrido `migrate:deploy` de nuevo el **2026-08-22**, se aplicó sola (era la única pendiente). Corrido `migrate:deploy` de nuevo el **2026-08-23**, se aplicó sola (era la única pendiente). Corrido `migrate:deploy` de nuevo el **2026-08-24**, se aplicó sola (era la única pendiente). La próxima libre es la **`0027`**. ⚠️ **La `0020` (y ahora también la `0023`) reemplazan `app.nap_publico`, así que la `0014` ya no puede aplicarse después de ninguna de las dos**: sería reponer la allowlist vieja y perder `bienvenida`, `destacados` y `testimonios` en silencio. Nadie produce ese orden —`migrarConRegistro` aplica las pendientes **ordenadas**— y el test de independencia de orden se reescribió para modelar el real. La lección, que vale para la próxima que reemplace una función: **una migración solo se puede reordenar contra las que no tocan lo mismo que ella.** **Historial de tropiezos que siguen valiendo:** la `0014` aplicó en la posición que su test predecía —entre la `0012` y la `0015`, no al final— y **el `grant` sobrevivió al `drop column`**, verificado donde importa: el renderizador sirve «La Birra Bar» con las dos direcciones del footer, y ese dato solo puede venir de `business_profile_publico`. **Desde el 2026-08-10 ese sitio ya no responde en `amg-renderer-production.up.railway.app`**, sino en `birrabar.bigballs.es`: la URL del servicio se estaba usando como dominio de cliente, y ahí el sitio era **indexable** (el `noindex` cubre `*.bigballs.es`, no el dominio de Railway). **El primer intento de la `0018` falló** (`must be able to SET ROLE "app_barrido"`) y se revirtió sola: en PGlite el rol que migra es superusuario y en Supabase alojado no, así que `alter … owner to` exigía dos permisos que nadie había concedido. Arreglado y con un test que aplica esa migración **como rol no-superusuario** — sin él, la clase entera volvería a escaparse |

| **ADRs** | 24 (la `ADR-24`, membresías escribibles bajo RLS, aceptada el 2026-08-02), más 4 observaciones — **las 4 cerradas**: `OBS-04` (quién edita la web) se cerró el **2026-09-10** en **(a) edita solo la agencia**, y con eso **desbloquea reescribir ADR-11** |
| **Reviews externas** | **16 rondas** (Codex), **22 tandas** de correcciones — la 13ª fue la primera sobre el arnés `.claude/`, la 14ª la primera sobre un **documento de diseño** (la spec de KR-2), la 15ª la primera **híbrida** (código + el plan de la plataforma) y la 16ª un fix round de código sobre el sub-proyecto 1 **ya cerrado y mergeado** (2 hallazgos, ambos verificados y corregidos con un commit directo sobre `main`). El detalle, tanda por tanda, en [08-testing-calidad.md](08-testing-calidad.md#revisiones-externas-codex--qué-encontraron-y-qué-se-corrigió) |
...
- **El polling del brief (4 s) es a ojo**, y la lista de runs no pollea. Se calibra con la duración
  real de una corrida.
- ~~**ADR-11 (offboarding) sigue sin poder firmarse.**~~ 🟡 **REESCRITA el 2026-09-11, casi firmable.**
  De los tres bloqueos que tenía, dos cayeron: **OBS-04 se cerró** en (a) —así que "editable" nombra,
  sin ambigüedad, una capacidad que el cliente **gana en la baja**— y la **"salida gestionada" tiene
  forma** (pago único + cuota mensual). El ADR tiene ahora una sección *«versión vigente»* que es la
  que se lleva a un contrato, y el cuerpo viejo quedó marcado como historia. Y el tercer bloqueo cayó
  el mismo día: **el snapshot estático ya existe y está verificado** (`npm run snapshot -w renderer`,
  imágenes descargadas, cero terceros comprobado en un navegador). **Queda UNA sola cosa, y no es
  código: los dos importes de Juan.** De paso, **(b1) "el cliente lo hostea" se retiró de la oferta**: el renderizador es
  multi-tenant y lee de la base de AMG. Se retiró como *"hoy no se ofrece"*, no como *"no se puede"* —
  `demo-server.ts` ya corre el renderizador contra PGlite sembrado desde un JSON, así que las piezas
  existen; falta el empaquetado.
- **🔴 `brand.tema` no cruza las fronteras: HOY NINGÚN CLIENTE PUEDE TENER MODO OSCURO.** No es nueva:
  **declarada como deuda el 2026-08-10** ([`15` § Bloque E, Etapa 1](15-plan-plataforma.md)) y **no
  arreglada desde entonces**; el navegador del snapshot volvió a encontrarla el 2026-09-11, y por eso
  sube acá con dueño en vez de quedarse en una nota de una etapa cerrada. **El dato que importa no es
  que se descubrió: es que se conocía y se dejó pasar un mes.**

  `ensamblarCss` (`web-builder/src/render/css.ts:355`) hace viajar el CSS oscuro **solo si
  `brand.tema === "auto"`**, y `tema` **no cruza tres de las cuatro fronteras**: no está en el
  `brandSchema` de Zod (`web-builder/src/contract.ts`), no lo construye la allowlist de
  `app.nap_publico` (ninguna de las migraciones `0009`/`0010`/`0014`), y no lo copia `perfilValido`
  (`renderer/src/perfil.ts`). Medido por los dos caminos —con la columna generada de Postgres y
  saltándosela con un doble en memoria— y en los dos el `<style>` sale **sin un solo
  `prefers-color-scheme`**.

  Es exactamente el patrón que este proyecto ya nombró: **una capacidad que el código tiene, la ficha
  acepta y ningún cliente puede recibir**. Y es la clase de cosa que un test no caza porque los tests
  del CSS construyen el `brand` en TypeScript, saltándose las tres fronteras que lo descartan. **El
  snapshot no lo empeora ni lo arregla**: su `<style>` es idéntico byte a byte al del sitio vivo (lo
  fija el test de paridad), así que el día que `tema` cruce las fronteras, el snapshot hereda el
  oscuro sin tocar una línea. Arreglarlo son los tres arreglos que la nota del `15` ya enumeraba:
  migración de la allowlist, Zod y `perfilValido` — trabajo de `datos` + `render`, no del Bloque H.
...
> ### ✅ Actualización (2026-09-10): el checklist de Juan queda cerrado, salvo dos números
>
> Recorrido entero con Juan, decisión por decisión ([`16-pendientes-juan.md`](16-pendientes-juan.md)):
>
> | Ítem | Decisión |
> |---|---|
> | **#4 `CACHE_TTL_MS`** | **60 s** (`CACHE_TTL_MS=60000`). Falta que Juan lo ponga en Railway |
> | **#5 Dominios custom** | **Esperar**, con disparador escrito: se sube el plan el día que se firme un **tercer** cliente con dominio propio |
> | **#6 OBS-04** | **(a) edita solo la agencia.** Si algún día edita un cliente, el camino es **(c)** desde el portal, nunca **(b)** un seat en Storyblok. **Cierra OBS-04 y desbloquea ADR-11** |
> | **#7 Salida gestionada** | **Pago único + cuota mensual**, con mínimo alto explícito. **Los dos números siguen pendientes de Juan** |
> | **#2 Business Profile API** | **En pausa**: no se manda la solicitud por ahora. El módulo 3 queda como demo permanente mientras dure |
>
> **Dos hallazgos salieron de tomar estas decisiones**, los dos anotados en su bloque del plan:
>
> - **El TTL no era lo que este documento creía.** El webhook de invalidación lo dispara **solo
>   Storyblok**, pero la home, `/menu`, la nav y el pie se sintetizan desde `business_profile`
>   (Postgres) y se hornean en el HTML cacheado — así que **un cambio hecho desde el portal no
>   invalida nada** y aparece sólo al vencer el TTL. El TTL era el tiempo de respuesta del editor, no
>   sólo el techo de propagación entre instancias.
> - **⚠️ Hay una trampa armada en producción.** `crearFuncionPollingResenas` está registrada
>   incondicionalmente; el día que alguien pulse «Conectar Google» estando en `GOOGLE_REVIEWS_MODO=mock`
>   se insertan **dos reseñas inventadas en la base real**, se dispara una **alerta de Telegram de
>   verdad** y se gasta una **llamada real a OpenAI**. Acotado (el `googleReviewId` del mock es
>   determinista) pero permanente. Como el trámite quedó en pausa, la trampa **no caduca sola** — así
>   que ✅ **se desarmó el mismo día**: el guardarraíl corta en `conectar` (antes de acuñar el `state`)
>   y en el `callback` (antes de toda escritura), con la regla calculada en `api/src/deps.ts` y el
>   campo obligatorio en `ApiDeps` para que el typecheck no deje ningún composition root sin decidir.
>   Ver el resumen ejecutivo y el Bloque F del plan.
>
> **El orden de trabajo que se venía siguiendo** —lote corto de cierre y después comparativas de
> seguros— ya está completo; el detalle histórico sigue en
> [`15-plan-plataforma.md` § El orden que recomiendo](15-plan-plataforma.md#el-orden-que-recomiendo-y-por-qué).
...
| **Export estático / offboarding** | ADR-11 | ✅ **Hecho — política y export, los dos (2026-09-11).** ADR-11 tiene su sección *«versión vigente»*: snapshot estático incluido, salida gestionada de pago (pago único + cuota mensual, **importes pendientes de Juan — lo único que queda para firmarlo**), y **(b1) «el cliente lo hostea» retirado** de la oferta como *"hoy no se ofrece"*, no como *"no se puede"*. Y el export **existe**: `npm run snapshot -w renderer -- <dominio> <destino>` produce `index.html`, `<slug>/index.html`, `_assets/fonts/` y `_assets/img/` con **las imágenes descargadas**, así que el entregable no depende de ninguna cuenta viva de Storyblok. Renderiza con **el mismo código que sirve el sitio vivo** —la decisión «qué HTML le toca a cada slug» se extrajo a `renderer/src/pagina.ts` y la comparten los dos, con un test que compara los dos caminos—, porque un snapshot que se vea distinto sería otro producto. Verificado **en un navegador**: 6 páginas, 10 imágenes, 7 tipografías, **cero peticiones a hosts externos**, **cero JavaScript ejecutable** — los únicos `<script>` son el JSON-LD y el `<script type="application/json" id="research-trace">` de las landings, los dos que el sitio vivo ya emite. Tres límites, dichos en el ADR porque son del entregable: hace falta un hosting estático (`file://` no, las rutas son absolutas a propósito para no reescribir enlaces); si una imagen no baja el snapshot se entrega igual pero el comando sale con código `2` y deja `_snapshot-informe.txt`; y el directorio de destino no se limpia. |
...
| **Estimaciones del presupuesto** | `lib/budget.ts` | ✅ **Recalibradas el 2026-08-22** contra `datasets/keywords.json` (23 keywords, corrida real). El default anterior subestimaba el enriquecimiento ~60% (dirección peligrosa para un preflight); el nuevo cubre el gasto real medido con ~17% de margen, fijado por test. `dfsSerp` no se tocó (ya sobreestimaba). Detalle en [15-plan-plataforma.md § Bloque D](15-plan-plataforma.md#bloque-d--calibrar-el-research-kr-1). |
...
**✅ Cerrada el 2026-09-10 en (a): edita solo la agencia.** Durante el servicio el cliente **no
edita** — ve su web y aprueba desde el portal en modo lectura (ADR-20). Con eso los **seats de
Storyblok quedan pocos y fijos**, que era la incógnita que impedía estimarlos (ver «Costo de
Storyblok» al final de este documento), y **ADR-11 se puede reescribir**: *handoff editable* nombra
ahora una capacidad que el cliente **gana en la baja**, no una que ya tenía.

Si algún día un cliente tiene que editar, **el camino es (c) —desde el portal, bajo nuestro RBAC— y
nunca (b)**, un seat suyo en Storyblok; se nombró con la decisión fría para que el día de la presión
nadie elija (b) por ser la de una tarde. **(c) no es un flag:** el rol `cliente` hoy no puede escribir
nada en `clients` (`app.puede_escribir()` es `maestro/equipo/servicio`,
[`0001_init.sql:387-390`](../../db/migrations/0001_init.sql#L387-L390)), así que abrirlo es una
migración de escritura acotada por columna con su tanda de tests de aislamiento — el mismo terreno
donde la `0021`/`0022` ya encontraron fugas reales.

Lo que **no** cierra esta decisión y queda como trabajo de roadmap: el botón *"Editar la web"* que
firme el enlace de preview al vuelo (retira la URL de larga duración pegada en el space, el eslabón
débil que esta observación identificó) y el clic-para-editar del Visual Editor.
...
~~**El número de seats no se puede estimar hasta cerrar OBS-04**~~ — ✅ **ya es estimable
(2026-09-10)**. OBS-04 se cerró en **(a): edita solo la agencia**, así que los seats son **pocos y
fijos** y no crecen con la cartera. Los *spaces* sí siguen creciendo —uno por cliente, como ADR-04
exige para un offboarding limpio—, y ése es el término que hay que llevar a la propuesta.
