...
| **Tests** | **1521** — 1177 en el monorepo + 344 en el portal (237 `node:test` + 107 Karma). Los de seguridad, contra Postgres real. Medido el **2026-08-08** con `npm run verificar` tras la 15ª review, los bloques A (**A4 incluido**), B y **C entero**, y las **entregas 1 y 2 del bloque E** (venía de 1199 → … → 1502 → 1521). El portal, con `-- --con-portal` y Karma aparte. |
| **Migraciones** | **18 en `main` y las 18 aplicadas en producción** (`0001`..`0012` + `0014`..`0019`), la `0018` y la `0019` el **2026-08-08**, la **`0014` el 2026-08-08 por la noche**. Ninguna pendiente. La próxima libre es la **`0020`**: la `0013` sigue **reservada** para la pieza 3 del portal (Ideas). **La `0014` aplicó en la posición que el test de orden predecía** —entre la `0012` y la `0015`, no al final— y **el `grant` sobrevivió al `drop column`**, verificado donde importa: `amg-renderer-production.up.railway.app` sirve «La Birra Bar» con las dos direcciones del footer, y ese dato solo puede venir de `business_profile_publico`. **El primer intento de la `0018` falló** (`must be able to SET ROLE "app_barrido"`) y se revirtió sola: en PGlite el rol que migra es superusuario y en Supabase alojado no, así que `alter … owner to` exigía dos permisos que nadie había concedido. Arreglado y con un test que aplica esa migración **como rol no-superusuario** — sin él, la clase entera volvería a escaparse |
...
| ✅ | **1177 tests en verde** (+344 en el portal: 237 `node:test` + 107 Karma) + typecheck limpio en los 7 paquetes. Los de seguridad, contra Postgres real. |
