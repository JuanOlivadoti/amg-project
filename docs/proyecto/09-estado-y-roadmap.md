...
| **Tests** | **1217** — 898 en el monorepo + 319 en el portal (225 `node:test` + 94 Karma). Los de seguridad, contra Postgres real. Medido el **2026-08-07** con `npm run verificar` tras la 15ª review (venía de 880: +2 de la guarda de `finishRun`, +11 del barrido en `db`, +5 en `orchestrator`); el portal con `-- --con-portal` y Karma aparte, sin cambios. |
