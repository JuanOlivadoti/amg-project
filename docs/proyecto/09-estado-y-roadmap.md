...
> [15-plan-plataforma.md § Bloque F](15-plan-plataforma.md#bloque-f--módulo-3-respondedor-de-reseñas-de-google).
> **Pendiente, fuera de esta sesión:** desplegar la migración `0026` a producción y que Juan cree el
> bot real con `@BotFather` (`TELEGRAM_BOT_TOKEN`/`TELEGRAM_MODO=live`). El resto de la fase 2 (acceso
> real a Google, limpieza de token revocado) sigue bloqueado por el trámite externo de la Business
> Profile API — ya no depende de elegir proveedor de alertas, eso quedó resuelto.
>
