...
| **Tests** | **1693** — 1349 en el monorepo + 344 en el portal (237 `node:test` + 107 Karma). Los de seguridad, contra Postgres real. Medido el **2026-08-09** con `npm run verificar` tras la 15ª review, los bloques A (**A4 incluido**), B y **C entero**, el **bloque E completo** (entregas 1, 2 y 3, con el gate de paridad re-capturado al cerrar) y las **etapas 1-3 del bloque J** (Ideas: la `0013`, `PgIdeas` y los tres endpoints) (venía de 1199 → … → 1564 → 1612 → 1659 → 1693). El portal, con `-- --con-portal` y Karma aparte. |
