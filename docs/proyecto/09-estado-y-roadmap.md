...
| **Tests** | **1245** — 914 en el monorepo + 331 en el portal (230 `node:test` + 101 Karma). Los de seguridad, contra Postgres real. Medido el **2026-08-07** con `npm run verificar` tras la 15ª review y el bloque A del plan (venía de 880: +2 de la guarda de `finishRun`, +16 del barrido, +12 de la sonda de `/_health`); el portal con `-- --con-portal` y Karma aparte, sin cambios. |
| **Migraciones** | 16 en `main` (`0001`..`0012` + `0015`..`0018`) · **15 aplicadas en producción y la `0018` PENDIENTE**. Las 15 se verificaron el 2026-08-07 por consulta y no por el "✔" del comando: los cuatro `check` de la `0017` existen y **14/14 páginas** quedaron con `seo.meta_title`, `seo.canonical` y `content_brief.h1`. La **`0018`** (barrido de runs colgados) se escribió ese mismo día y **todavía no se desplegó**; al aplicarla, lo primero a mirar es que `alter function … owner to app_barrido` pase — si el rol que migra no tiene `CREATEROLE`, **falla ruidosamente**, que es lo que se quería. La próxima libre es la **`0019`**: `0013` y `0014` siguen **reservadas** —la `0013` para la pieza 3 del portal (Ideas)— |
...
|---|---|---|
| **Desplegar la `0018`** (barrido de runs colgados) | `npm run migrate:deploy -w db` toca Supabase real, así que lo corre él. **Un solo paso** —no necesita re-seed, a diferencia de la `0017`— y hasta que se aplique, el barrido está en el código y **no corre**: el orquestador arrancaría llamando a una función que no existe. **Mirá que `alter function … owner to app_barrido` pase**; si el rol que migra no tiene `CREATEROLE`, la migración aborta ruidosamente (y eso es lo correcto) | — |
