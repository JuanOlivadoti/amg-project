...
- ~~**ADR-11 (offboarding) sigue sin poder firmarse.**~~ 🟡 **REESCRITA el 2026-09-11, casi firmable.**
  ...
  que se lleva a un contrato, y el cuerpo viejo quedó marcado como historia. **Queda por hacer
  exactamente dos cosas**: que Juan ponga los **dos importes**, y **verificar el snapshot estático
  como entregable** — lo único de código, y la razón por la que todavía no se firma: se puede sacar de
  `renderStory()` pero **nadie lo hizo nunca**, así que hoy esa línea del contrato describe una
  intención. De paso, **(b1) "el cliente lo hostea" se retiró de la oferta**: el renderizador es
...
| **Export estático / offboarding** | ADR-11 | 🟡 **La POLÍTICA está escrita (2026-09-11), el export NO.** ADR-11 tiene su sección *«versión vigente»*: snapshot estático incluido, salida gestionada de pago (pago único + cuota mensual, importes pendientes de Juan), y **(b1) «el cliente lo hostea» retirado** de la oferta — el renderizador es multi-tenant y lee de la base de AMG. Lo que sigue ⏳ **pendiente es el código**: **nadie produjo nunca un snapshot estático**. Se puede sacar de `renderStory()`, pero no existe el comando ni se comprobó que el resultado abra en un navegador desconectado de AMG. |
