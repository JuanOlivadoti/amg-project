...
>
> **La migración de este spec es la `0023`** (la próxima libre en `main` al escribir esto, tras el
> merge de `feature/resenas-google` que ocupó la `0021` y la `0022`). Confirmar el número libre real al
> implementar, no asumirlo de este documento si pasó tiempo — es exactamente el tipo de dato que se
> desactualiza con otra rama en curso.
...
| 2. Allowlist Postgres | migración `0023` (reemplaza `app.nap_publico`) | `video`, `alergenos`, `etiquetas`, `nutricion`, `precios[].comensales` — cada lista con su tope (14 / 7 / 1) |
