...
| `GOOGLE_REVIEWS_MODO` | `mock` | Modo del módulo de reseñas. **Decide si un endpoint responde 409**: con `mock` en un despliegue de **producción**, `POST /clients/:id/google/conectar` y `GET /clients/:id/google/callback` se bloquean — conectar en mock sembraría reseñas inventadas en la base real, dispararía alertas de Telegram de verdad y gastaría llamadas a OpenAI. ⚠️ **No la pongas en `live` para desbloquearlo**: `leerConfig` acepta el valor pero `getGoogleOAuthProvider("live")` **lanza** (el provider real no existe todavía) y la API no arrancaría. |
| `PORT` | `3000` | |
